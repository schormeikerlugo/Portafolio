...
function PlasmaDisk() {
  ...
  const uniforms = useMemo(() => ({
    uTime: { value: 0 },
    uColorStart: { value: new THREE.Color('#ffc107') }, // Amber/Gold
    uColorEnd: { value: new THREE.Color('#b71c1c') },   // Deep Red
  }), []);
  ...
  return (
    <mesh ref={meshRef} rotation={[-Math.PI / 2.5, 0, 0]}>
      ...
      <shaderMaterial
        ...
        transparent={true}
        depthWrite={false}
        blending={THREE.NormalBlending} // Changed from Additive to Normal for SOLID look
...
function BlackHoleModel() {
  ...
  return (
    <group scale={scale}>
      ...
      {/* Particles (Plasma Sparks) - Restored! */}
      <group rotation={[Math.PI / 2, 0, 0]}> {/* Align particles to disk */}
        <group rotation={[-Math.PI / 2.5, 0, 0]}> {/* Match disk tilt */}
